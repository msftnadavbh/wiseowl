#!/usr/bin/env python3
"""Install Wise Owl plugin assets into Codex-discovered locations."""
...
def plan_install(
    ...
    source_skill = plugin_root / "skills" / "wise-owl"
    source_agents = plugin_root / "assets" / "agents"

    for path in source_skill.rglob("*"):
        if path.is_file():
            planned[skill_dir / path.relative_to(source_skill)] = path.read_text(encoding="utf-8")

    for name in AGENT_NAMES:
        filename = f"{name}.toml"
        agent_model = agent_model_for(name, model, prime_model)
        planned[agents_dir / filename] = replace_model((source_agents / filename).read_text(encoding="utf-8"), agent_model)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install Wise Owl plugin assets.")
...
def main() -> int:
    ...
    plugin_root = Path(os.environ.get("WISE_OWL_PLUGIN_ROOT", plugin_root_from_script())).resolve()
    changed = write_planned(
        plan_install(
            args.scope,
            repo_root,
            codex_home,
            ...
            args.model,
            args.prime_model,
            args.patch_agents_md,
            user_skills_home,
        ),
        args.dry_run,
        args.force,
    )
    _, agents_dir, _ = target_paths(args.scope, repo_root, codex_home, user_skills_home)
    changed.extend(handle_legacy_agents(agents_dir, args.dry_run, args.force))

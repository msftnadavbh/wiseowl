...
- `blocked`: at least one finding has severity `blocking`
...
`evidence` must be one non-empty string citing a file path, function, test, command output, diff hunk, or stated constraint. Do not return evidence as an array.
...
A critic verdict must match its finding severities. A pass packet with findings, a concerns packet without findings, or a blocked packet without a blocking finding is malformed.
...
Accepted-finding `evidence` must be one non-empty string. Do not return evidence as an array. Use `required_builder_action`; `required_fix` is not a valid field.
...
Duplicate critic roles are invalid. Without `--critics` or `--mode`, Prime validation remains syntax/schema-only and does not claim source accounting.

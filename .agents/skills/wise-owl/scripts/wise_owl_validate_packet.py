...
def validate_source_accounting(errors: list[str], packet: dict[str, Any], critic_packets: list[dict[str, Any]] | None) -> None:
    seen: set[str] = set()
...
def validate_prime(packet: Any, critic_packets: list[dict[str, Any]] | None = None) -> list[str]:
    errors: list[str] = []
    ...
    validate_source_accounting(errors, packet, critic_packets)
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not errors:
        if args.critics and args.type != "prime":
            ...
            if not errors:
                errors = validate_prime(packet, critic_packets or None)
    ...
    if args.type == "prime" and not args.critics:
